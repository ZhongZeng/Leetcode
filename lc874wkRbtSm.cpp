...
public:
    int robotSim(vector<int>& commands, vector<vector<int>>& obstacles) {
        int x=0, y=0, d=0, td, mx=0;// d:N-0, E-1, S-2, W-3 
        unordered_map<int,unordered_set<int>> ux;
        unordered_map<int,unordered_set<int>>::iterator umi;
        
        for( int i=0; i<obstacles.size(); i++){
            umi=ux.find(obstacles[i][0]);
            if(umi!=ux.end()){
                umi->second.emplace(obstacles[i][1]);
            }else{
                unordered_set<int> us;
                us.emplace(obstacles[i][1]);
                ux.emplace(obstacles[i][0],us);
            }           
        }
                    ...
                    switch(d){
                        case 0:
                            for( int j=0; j<commands[i]; j++){
                                if(isObs(x,y+1,ux)) break;
                                else    y++;
                            }
                            break;
                        case 1:
                            for( int j=0; j<commands[i]; j++){
                                if(isObs(x+1,y,ux)) break;
                                else    x++;
                            }              
                            break;
                        case 2:
    ...
    bool isObs( int x, int y, unordered_map<int,unordered_set<int>>& ux){
        unordered_map<int,unordered_set<int>>::iterator umi=ux.find(x);
        
        if(umi!=ux.end())   return umi->second.find(y)!=umi->second.end();
        else    return false;
