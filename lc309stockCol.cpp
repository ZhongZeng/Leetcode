...
Next challenges: Edit Distance, Arithmetic Slices II - Subsequence, Strange Printer
...
Runtime: 8 ms
Your runtime beats 25.80 % of cpp submissions.
...
public:
    int maxProfit(vector<int>& prices){
        // inspired by house robber 
        // I tried unnecessary complicated methods 
        if(prices.size()<2) return 0;
        
        int i, p1, p2, p3=0, pt, ps;
        p2=p3+prices[0]<prices[1]?prices[1]-prices[0]:0;
        if(prices.size()<3) return p2;
        p1=p2+(prices[1]<prices[2]?prices[2]-prices[1]:0);
        //cout<<p3<<","<<p2<<","<<p1<<endl;
        
        for(i=3; i<prices.size(); i++){
            if(prices[i-1]<prices[i]){
                if(prices[i-3]<prices[i-2]&&prices[i-1]<prices[i-2]){
                    pt=p1;
                    ps=p3+prices[i]-prices[i-1];
                    if(prices[i-2]<prices[i])   p1=ps<p2+prices[i]-prices[i-2]?p2+prices[i]-prices[i-2]:ps;
                    else    p1=ps<p2?p2:ps;
                    p3=p2;
                    p2=pt;
                }else{// prices[i-3]>=prices[i-2]
                    p3=p2;
                    p2=p1;
                    p1=p1+prices[i]-prices[i-1];
                }
            }else{
                p3=p2;
                p2=p1;
...
/*
// Runtime: 9 ms
// Your runtime beats 11.51 % of cpp submissions.
class Solution {
public:
    int maxProfit(vector<int>& prices){
		// O(n) time, O(1) space 
        ...
        for(i=3; i<prices.size(); i++){
            if(prices[i-1]<prices[i]){
                if(prices[i-3]<prices[i-2]&&prices[i-1]<prices[i-2]){
                    pt=p1;
                    p1=maxV(p3+prices[i]-prices[i-1], p2, p2+prices[i]-prices[i-2]);
                    p3=p2;
                    p2=pt;
                }else{// prices[i-3]>=prices[i-2]
                    p3=p2;
...
public:
    int maxProfit(vector<int>& prices){
        ...
        int i, p1=0, p2=0, p3=0, b3;// i, profit, sell time
        vector<int> vc;
        vc.push_back(prices[0]);
        for(i=1; i<prices.size(); i++){
            if(vc.size()<2){// ==1
                if(vc[0]<prices[i]) vc.push_back(prices[i]);
                else    vc[0]=prices[i];
            }else{// ==2
                if(prices[i]<vc[1]) break;
                else    vc[1]=prices[i];
            }
        }
        if(1<vc.size()){// ==2
            p1+=prices[vc[1]]-prices[vc[0]];
            p2+=prices[vc[1]-1]-prices[vc[0]];
            b3=prices[vc[0]];
        }else{
            return 0;
        }
        ...
        for( vc.push_back(prices[i]); i<prices.size(); i++){
            if(vc.size()<4){// ==3
                if(vc[2]<prices[i]) vc.push_back(prices[i]);
                else    vc[2]=prices[i];
            }else{// ==4
                if(prices[i]<vc[3]){
                    if(vc[1]+1<vc[2]){
                        p1=p1+prices[vc[3]]-prices[vc[2]];
                        p2=p1+prices[vc[3]-1]-prices[vc[2]];
                        p3=p1;
                        b3=prices[vc[2]];
                    }else{
                        p1=minV(p1+prices[vc[3]]-prices[vc[2]+1],p2+prices[vc[3]]-prices[vc[2]],p3+prices[vc[3]]-b3);
                        p2=minV(p1+prices[vc[3]-1]-prices[vc[2]+1],p2+prices[vc[3]-1]-prices[vc[2]],p3+prices[vc[3]-1]-b3);
                        
                        //p1-prices[vc[2]+1], p2-prices[vc[2]], p3-b3
                        if(p1-prices[vc[2]+1]<p2-prices[vc[2]]){
                            if(p3-b3<p2-prices[vc[2]]){
                                p3=p2;
                                b3=prices[vc[2]];
                            }
                        }else{
                            if(p3-b3<p1-prices[vc[2]+1]){
                                p3=p1;
                                b3=prices[vc[2]+1];
                            }
                        }
...
public:
    int maxProfit(vector<int>& prices) {
        ...
        if(prices.size()<2) return 0;
        int p=0, m;
        if(prices[0]<prices[1]) p+=prices[1]-prices[0];
        if(2<prices.size()&&prices[1]<prices[2])    p+=prices[2]-prices[1];
        if(prices.size()<4) return p;
        
        for(int i=3; i<prices.size(); i++){
            if(prices[i-1]<prices[i]){
                p+=prices[i]-prices[i-1];
                if(prices[i-3]<prices[i-2]&&prices[i-1]<prices[i-2]){
					m=minV(prices[i-2]-prices[i-3],prices[i]-prices[i-1],prices[i-2]-prices[i-1]);
					p-=m;
                }
            }
        }
...
protected:    
    int maxProfit(vector<int>& prices, int s, int pf, int buy){
		cout<<s<<endl;
        if(buy!=-1){ // not holding any stock
            if(prices.size()-1==s)  return prices[s]-buy+pf;
            if(buy<prices[s]){
                // 1:sell, 2:not sell
                int pf1=maxProfit(prices, s+2, pf+prices[s]-buy, -1), pf2=maxProfit(prices, s+1, pf, buy);
                return pf1<pf2?pf2:pf1;
            }else{
                return maxProfit(prices, s+1, pf, buy);
            }
        }else{ // holding stock
            if(prices.size()-2<s)   return pf;
            if(prices[s+1]<prices[s]){
				return maxProfit(prices, s+1, pf, -1);
            }else{
				// 1:buy, 2:not buy
                int pf1=maxProfit(prices, s+1, pf, prices[s]), pf2=maxProfit(prices, s+1, pf, -1);
                return pf1<pf2?pf2:pf1;                
            }            
        }
    }
...
int main(){
	Solution s;
	vector<int> vc;
	vc={70,4,83,56,94,72,78,43,2,86,65,100,94,56,41,66,3,33,10,3,45,94,15};
