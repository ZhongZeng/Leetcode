...
// Runtime: 24 ms, faster than 99.87% of C++ online submissions for Flatten a Multilevel Doubly Linked List.
class Solution {
public:
    Node* flatten(Node* head) {
        // stack, equivalent to pre-order DFS; return root of single level, double linked list 
        if(!head)   return head;
        vector<Node*> vn;
        Node* rt=head, * nd=head;
        if(nd->next)    vn.push_back(nd->next);// first node
        if(nd->child)   vn.push_back(nd->child);
        nd->child=NULL;
        ...
        while(!vn.empty()){
            ...
            nd=vn.back();
            vn.pop_back();
            if(nd->next)    vn.push_back(nd->next);
            if(nd->child)   vn.push_back(nd->child);
            nd->child=NULL;
        }
        return rt;
...
// Runtime: 24 ms	Your runtime beats 98.96 % of cpp submissions.
// Runtime: 28 ms	Your runtime beats 9.38 % of cpp submissions.
class Solution {
public:
